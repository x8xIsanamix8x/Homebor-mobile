...
if($filterP == 'No') {
    ...
    while($start = $query_d->fetch(PDO::FETCH_ASSOC)) {
            $response['data'] = $start;
            $cont = 0;
            $sql = "SELECT payments.*, pe_student.name_s, pe_student.l_name_s, pe_student.mail_s, pe_student.photo_s, DATE_FORMAT(date_p, '%Y-%m-%d') FROM `payments` INNER JOIN pe_student ON payments.r_mail = '$userLogin' AND payments.reserve_s = pe_student.mail_s ORDER BY id_p DESC";
