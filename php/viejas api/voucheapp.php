...
if($filterP == 'No') {
    ...
    while($start = $query_d->fetch(PDO::FETCH_ASSOC)) {
            $response['data'] = $start;
            $cont = 0;
            $sql = "SELECT vouche.*, manager.id_m, manager.photo FROM vouche INNER JOIN manager ON vouche.email = '$userLogin' AND vouche.id_m = manager.id_m ORDER BY vouche.dates DESC;";
            $query = $result->query($sql);
            while($data = $query->fetch(PDO::FETCH_ASSOC)){
                $response["voucherlist"][$cont]["id_v"] = $data["id_v"];
                $response["voucherlist"][$cont]["title"] = $data["title"];
                $response["voucherlist"][$cont]["email"] = $data["email"];
                $response["voucherlist"][$cont]["user"] = $data["user"];
                $response["voucherlist"][$cont]["sender"] = $data["sender"];
                $response["voucherlist"][$cont]["dates"] = $data["dates"];
                $response["voucherlist"][$cont]["link"] = $data["link"];
                $response["voucherlist"][$cont]["photo"] = $data["photo"];
                $cont++;
            }
