...
while($start = $query_d->fetch(PDO::FETCH_ASSOC)) {
        $response['data'] = $start;
        $cont = 0;
        $sql = "SELECT notification.*, pe_student.mail_s, pe_student.photo_s FROM notification INNER JOIN users ON users.mail = '$userLogin' AND notification.user_r = users.mail INNER JOIN pe_student ON notification.user_i_mail = pe_student.mail_s ORDER BY id_not DESC";
