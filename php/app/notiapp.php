<?php
    ...
    while($start = $query_d->fetch(PDO::FETCH_ASSOC)) {
            $response['data'] = $start;
            $cont = 0;
            $sql = "SELECT notification.*, pe_student.mail_s, pe_student.photo_s, pe_student.id_m, manager.id_m, manager.mail, manager.photo, manager.a_name, DATE_FORMAT(max(date_), '%Y-%m-%d') as notiDay FROM notification INNER JOIN users ON users.mail = '$userLogin' AND notification.user_r = users.mail AND notification.state != 2 LEFT JOIN pe_student ON notification.user_i_mail = pe_student.mail_s LEFT JOIN manager ON notification.user_i_mail = manager.mail OR notification.user_i_mail = pe_student.mail_s AND pe_student.id_m = manager.id_m GROUP by id_not ORDER BY id_not DESC";
            ...
            while($data = $query->fetch(PDO::FETCH_ASSOC)){
                ...
                $response["notification"][$cont]["dateNoti"] = $data["notiDay"];
                $response["notification"][$cont]["des"] = $data["des"];
                $cont++;
            }
